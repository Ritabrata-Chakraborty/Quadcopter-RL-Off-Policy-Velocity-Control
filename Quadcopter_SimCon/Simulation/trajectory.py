...
def testVelControl(t):
    ...
    side_time = 3.0
    yaw_turn = pi                       # 180 deg
    yaw_duration = 2.0                  # seconds for 180 deg
    yaw_rate = yaw_turn / yaw_duration  # rad/s

    desPos[2] = -1.0  # Hold 1m altitude (NED: z down)

    t1 = side_time                      # end of side 1
    t2 = t1 + side_time                 # end of side 2
    t3 = t2 + yaw_duration              # end of yaw turn
    t4 = t3 + side_time                 # end of side 3
    t5 = t4 + side_time                 # end of side 4

    if t < t1:
        # Side 1: pitch forward (+x), yaw=0
        desVel[0] = speed
    elif t < t2:
        # Side 2: pitch sideward (+y via roll), yaw=0
        desVel[1] = speed
    elif t < t3:
        # Yaw turn: rotate 180 deg in place using angular velocity
        desEul[2] = (t - t2) / yaw_duration * yaw_turn
        desYawRate = yaw_rate
    elif t < t4:
        # Side 3: pitch forward (now facing -x after 180 deg yaw)
        desVel[0] = -speed
        desEul[2] = pi
    elif t < t5:
        # Side 4: roll sideward (-y via roll), yaw=pi
        desVel[1] = -speed
        desEul[2] = pi
    else:
        # Hover
