...
class MapGenerator:
    ...
    def generate(self, rng: random.Random, colors: dict[str, tuple[int, int, int]]) -> np.ndarray:
        ...
        Args:
            rng: Random number generator (controls randomness + reproducibility).
            colors: Color mapping with keys 'free', 'occupied', 'start'.

        Returns:
            Canvas-sized RGB array with obstacles and start patch.
        """
        cfg = self.config
        ...
        # Initialize canvas with border, free region in center
        ...
        # Reserve start position and place obstacles
        start_pos = self._reserve_start_position(rng)
        start_zone = self._get_reserved_zone(start_pos)
        self._place_obstacles(rng, img, free, occ, start_zone, margin)
        self._place_start(img, start_pos, start)

        return img

    def _reserve_start_position(self, rng: random.Random) -> tuple[int, int]:
    ...
    def _get_reserved_zone(self, start_pos: tuple[int, int]) -> tuple[int, int, int, int]:
        """Get bounding box of reserved zone (start patch + clearance)."""
        r, c = start_pos
        cfg = self.config
        return (
    ...
    def _place_obstacles(
        ...
        cfg = self.config
        target_area = int(cfg.inner_size * cfg.inner_size * rng.uniform(cfg.obs_min_density, cfg.obs_max_density))
        placed_area = 0
        max_attempts = 100
        ...
        while placed_area < target_area:
            ...
            for _ in range(max_attempts):
                ...
                if self._can_place_obstacle(img, r, c, side, free, start_zone):
    ...
    def _can_place_obstacle(
        ...
        """Check if obstacle can be placed: clearance zone free + no start zone overlap."""
        cfg = self.config
        ...
        no_start_overlap = not self._rects_overlap(
    ...
    def _rects_overlap(
        ...
        """Check if rectangle overlaps with zone (r_min, r_max, c_min, c_max)."""
        r_min2, r_max2, c_min2, c_max2 = zone
        return not (r_max1 <= r_min2 or r_min1 >= r_max2 or
                    c_max1 <= c_min2 or c_min1 >= c_max2)

    def _place_start(self, img: np.ndarray, pos: tuple[int, int], start: np.ndarray) -> None:
...
def generate_dataset(
    ...
    Args:
        train_count: Number of training maps.
        test_count: Number of test maps.
        train_dir: Directory to save training maps.
        test_dir: Directory to save test maps.
    ...
    print(
        f"  Canvas: {config.canvas_size}×{config.canvas_size} | "
        f"Free: {config.inner_size}×{config.inner_size} | "
        f"Margin: {margin}px"
    )
    print(
        f"  Start: {config.start_patch_size}×{config.start_patch_size} | "
